...
//Code Here
let postTaxPrices = prices.map((e, i) => e * 1.07);
...
//Code Here
let totalPopulation = populations.reduce((acc, curr, i, arr) => acc + curr)
...
let priceTax = () =>{ 
  return orders.map((e, i, arr) => {e["price"] + e["tax"]});}

let ordersTotal = priceTax();

ordersTotal.reduce((acc, elem) => acc + elem);
...
let calculator = () => {
  var one = purchases.filter((e, i, arr) => arr[i].owner==="Bob");
  var two = one.map((e, i, arr) => arr[i].price)
  var three = two.reduce((acc, curr) => acc + curr)
  return three;
}

let bobsTotal = calculator();
